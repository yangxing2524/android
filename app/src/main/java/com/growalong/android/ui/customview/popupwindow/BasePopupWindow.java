package com.growalong.android.ui.customview.popupwindow;
...
import com.growalong.android.util.AnimUtil;
import com.growalong.android.util.InputMethodUtils;
...
public abstract class BasePopupWindow implements BasePopup, PopupWindow.OnDismissListener, PopupController {
    ...
    private void tryToShowPopup(View v) {
        try {
            ...
            if (e instanceof WindowManager.BadTokenException) {
                retryToShowPopup(v);
                Log.e(TAG, "have no window token,retry to show");
                e.printStackTrace();
            } else {
    ...
    protected Animation getTranslateAnimation(int start, int end, int durationMillis) {
        return AnimUtil.getTranslateAnimation(start, end, durationMillis);
    }
    ...
    protected Animation getScaleAnimation(float fromX,
                                          ...
                                          int pivotYType,
                                          float pivotYValue) {
        return AnimUtil.getScaleAnimation(fromX, toX, fromY, toY, pivotXType, pivotXValue, pivotYType, pivotYValue);
    }
    ...
    protected AnimatorSet getDefaultSlideFromBottomAnimationSet() {
        return AnimUtil.getDefaultSlideFromBottomAnimationSet(mAnimaView);
    }
