...
import agora.openvcall.AgoraAGApplication;
import agora.openvcall.model.AgoraConstantApp;
import agora.openvcall.model.AgoraCurrentUserSettings;
import agora.openvcall.model.AgoraEngineConfig;
import agora.openvcall.model.AgoraMyEngineEventHandler;
...
public abstract class AgoraBaseActivity extends AppCompatActivity {
    ...
    public boolean checkSelfPermission(String permission, int requestCode) {
        ...
        if (Manifest.permission.CAMERA.equals(permission)) {
            ((AgoraAGApplication) getApplication()).initWorkerThread();
        }
        return true;
    }

    protected RtcEngine rtcEngine() {
        return ((AgoraAGApplication) getApplication()).getWorkerThread().getRtcEngine();
    }

    protected final AgoraWorkerThread worker() {
        return ((AgoraAGApplication) getApplication()).getWorkerThread();
    }

    protected final AgoraEngineConfig config() {
        return ((AgoraAGApplication) getApplication()).getWorkerThread().getEngineConfig();
    }

    protected final AgoraMyEngineEventHandler event() {
        return ((AgoraAGApplication) getApplication()).getWorkerThread().eventHandler();
    }
    ...
    protected AgoraCurrentUserSettings vSettings() {
        return AgoraAGApplication.mVideoSettings;
    }
    ...
    protected void initVersionInfo() {
        String version = "V " + BuildConfig.VERSION_NAME + "(Build: " + BuildConfig.VERSION_CODE
                + ", " + AgoraConstantApp.APP_BUILD_DATE + ", SDK: " + Constant.MEDIA_SDK_VERSION + ")";
